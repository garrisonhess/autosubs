...
def pad_collate_fn(data, mode='train'):
    ...
    audio, audio_lengths, subtitles, subtitle_lengths = list(zip(*data))
...
def split_into_val_train(config):
    """
    ...
    dataset = KnnwAudioDataset(knnw_audio_path, knnw_subtitle_path, KNNW_TOTAL_FRAMES, KNNW_TOTAL_DURATION, cfg['threshold'])
    ...
    train_loader = DataLoader(dataset = dataset, batch_size=config['batch_size'], num_workers=cfg['num_workers'], 
                              pin_memory=cfg['pin_memory'], drop_last = not cfg['DEBUG'], sampler=train_sampler, 
                              collate_fn=pad_collate_fn)
    val_loader = DataLoader(dataset = dataset, batch_size=cfg['val_batch_size'], num_workers=cfg['val_workers'], 
